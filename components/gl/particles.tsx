...
// Simple damping function to replace maath/easing.damp
function damp(obj: any, prop: string, target: number, smoothing: number, delta: number) {
  const current = obj[prop]
  const diff = target - current
  const dampedDiff = diff * (1 - Math.exp(-smoothing * delta * 60))
  obj[prop] = current + dampedDiff
}
